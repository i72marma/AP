...
Vamos a agregar un sistema de replicación de dispositivos de bloques conocido como DRBD, al que posteriormente añadirermos un recurso, que en nuestro caso será el directorio de datos del sitio web, que se mantendrá replicado y operativo en modo maestro/esclavo.
...
Con la ejecución de la receta de ansible, hemos creado un clúster entre los dos nodos con dos recursos:
...
Necesitaremos añadir dos discos identicos al nodo1 y nodo2 respectivamente. Se utilizarán para crear nuestro dispositivo de bloque DRBD. Necesitaremos pues instalar los paquetes necesarios en los dos nodos:
...
Crearemos el recurso drbd y lo activamos en ambos nodos:

    # drbdadm create-md wwwdata
    # drbdadm up wwwdata

Asignamos el nodo1 como primario y el nodo2 como secundario:
...
## Configuración del DRBD en el cluster de alta disponibilidad

A continuación vamos a crear dos nuevos recursos en pacemaker.

El primer recurso controlará el recurso drbd, el clúster decidirá que nodo se pone como primario y secundario en todo momento. Para realizar esta configuración vamos a usar una característica de pacemaker, los cambios lo vamos a realizar sobre un fichero (llamado `drbd_cfg`) y en la última instrucción vamos a aplicar todos los cambios al cluster desde este fichero, sería de esta forma:
...
Una vez terminada la configuración vemos el estado del clúster:
